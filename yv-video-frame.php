<?php
/*
	Plugin Name: Youtube, Vimeo Video Frame for WPM Only
	...
	Author: webpuzzlemaster.com
	Version: 2.1
	Author URI: http://webpuzzlemaster.com
*/


define( 'YTVF_VERSION', '2.1' );
...
class YTVF {
	...
	private static function init_hooks() {
		...
		add_action( 'wp_enqueue_scripts', ['YTVF', 'ytvf_load_admin_script'] );
	}
	
	static function ytvf_load_admin_script() {
	    
	}

	public static function get_video_thumb($_id, $type){
		if( $type == 'vimeo' ){
		    $data = file_get_contents("https://vimeo.com/api/v2/video/$_id.json");
		    if( $data !== FALSE ) {
			    $data = json_decode($data);
			    return ( is_array($data) ) ? '<img class="thumbs" src="'.$data[0]->thumbnail_large.'">' : '';
			} else {
				return 'empty';
			}
		} else {
	...
	public static function ytvf_shortcode($attr){
		ob_start();
		$type = ( isset($attr['type']) && $attr['type'] == 'vimeo' ) ? 'vimeo' : 'youtube';
		echo '<div class="ytvf ytvf_dark_frame ytvf_wrapper">';
			echo '<div class="ytvf_responsive_video_container">'; 
				echo SELF::get_video_thumb($attr['id'], $type);
				echo '<img class="playbutton" data-id="'.$attr['id'].'" data-type="'.$type.'" src="https://s.ytimg.com/yts/img/large_play_button_ringo-vfljWXIdx.png">';
			echo '</div>';
		echo '</div>';
